...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage()) {
            ...
            try {
                answerPreCheckoutQuery(preCheckoutQuery.getId(), true);
                sendMessage(chatId, "Платеж принят");
            } catch (Exception e) {
                e.printStackTrace();
                sendMessage(chatId, "Произошла ошибка при обработке платежа");
            }
        }
    }
    ...
    private void answerPreCheckoutQuery(String queryId, boolean ok) {
        try {
            execute(new AnswerPreCheckoutQuery(queryId, ok));
        } catch (TelegramApiException e) {
            e.printStackTrace();
        }
    }
